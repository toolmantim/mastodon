...
RSpec.describe UserSettings, type: :model do
  ...
  describe '#[]' do
    context 'when setting is not set' do
      it 'returns default value' do
        expect(subject[:always_send_emails]).to eq false
      end
    end
  ...
  describe '#[]=' do
    context 'when value matches type' do
      ...
      it 'updates value' do
        expect(subject[:always_send_emails]).to eq true
      end
    end
  ...
  describe '.definition_for?' do
    context 'when key is defined' do
      it 'returns true' do
        expect(described_class.definition_for?(:always_send_emails)).to eq true
      end
    end

    context 'when key is not defined' do
      it 'returns false' do
        expect(described_class.definition_for?(:foo)).to eq false
      end
    end
  end
end
