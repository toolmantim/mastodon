  ...
  describe '#render' do
    context 'account.approved' do
      ...
      it 'renders event-related variables into template' do
        expect(subject.render('foo={{event}}')).to eq 'foo=account.approved'
      end

      it 'renders event-specific variables into template' do
        expect(subject.render('foo={{object.username}}')).to eq "foo=#{object.username}"
      end

      it 'escapes values for use in JSON' do
        expect(subject.render('foo={{object.account.display_name}}')).to eq 'foo=Foo\\"'
      end
    end
  end
end
